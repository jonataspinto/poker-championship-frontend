...
import { createNewRound, revalidateListJourneys } from "@/services/actions";
  ...
  const [, dispatchAction, isPending] = useActionState(
    async (previousValue: Record<string, string>, formData: FormData) => {
      try {
        ...
        const playersList = formDataValues.reduce((acc, playerId) => {
        ...
        await createNewRound(seasonId, { players: playersList });
