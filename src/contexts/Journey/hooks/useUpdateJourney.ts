    ...
    async (journeyData: IJourney) => {
      ...
      try {
        ...
        draftStateJourneys.splice(
          state.journeys.findIndex((journey) => journey.uuid === data.uuid),
          1,
          {
            ...state.journeys.find((journey) => journey.uuid === data.uuid),
            ...data
          }
        );
  ...
  const closeJourney = useCallback(
    async (journeyId: string) => {
      ...
      try {
        ...
        draftStateJourneys.splice(
          state.journeys.findIndex((journey) => journey.id === journeyId),
          1,
          data
        );
